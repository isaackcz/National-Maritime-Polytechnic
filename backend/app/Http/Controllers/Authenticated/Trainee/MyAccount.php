...
class MyAccount extends Controller
{
    ...
    public function create_or_update_additional_info (Request $request) {
        ...
        if($validator->fails()) {
            ...
            try {
                ...
                $this_education_info->school_course_taken = $request->school_course_taken;
                $this_education_info->school_address = $request->school_address;
                $this_education_info->school_graduated = $request->school_graduated;
                ...
                // dd($this_trainee_files_info);
                // dd($request->hasFile('file_e_signature'));
                //e-signature
                if ($request->hasFile('file_e_signature')){
                    // dd($this_trainee_files_info->file_e_signature);
                    $this_trainee_files_info->file_e_signature = $this->savefile($request->file_e_signature, $this_trainee_files_info->file_e_signature);
                }
                //file_id_picture
    ...
    public function savefile($fileUploaded, $this_trainee_files) {
        if($fileUploaded){
            $filePath = public_path('trainee-files/' . $this_trainee_files);

            if ($fileUploaded && file_exists($filePath) && is_file($filePath)) {
                unlink($filePath);
            }
    ...
    public function get_all_courses_and_schools (Request $request) {
        $courses = MainCourse::where('status', 'ACTIVE')->get();
        $schools = MainSchool::where('status', 'ACTIVE')->get();
